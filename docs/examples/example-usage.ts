...
async function discoverMultipleConnections(
  myProfile: UserProfile,
...
async function compareConnectionStrategies(
  myProfile: UserProfile,
  targetProfile: UserProfile,
  networkGraph: Graph
): Promise<void> {
  ...
  return `
    <div className="connection-strategy-card">
      ...
      <div className="next-steps">
        <h3>Next Steps</h3>
        <ol>
          ${strategy.nextSteps.map((step, i) => `<li>${step}</li>`).join('')}
        </ol>
      </div>
...
async function trackConnectionAttempt(
...
export async function main() {
  ...
  const mockGraph: Graph = {
    ...
    bidirectionalBFS: async (source: string, target: string) => null,
  };
