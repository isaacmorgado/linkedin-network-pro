      ...
      {keywords.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {keywords.slice(0, 20).map((keyword) => (
            <KeywordBadge key={keyword.term} keyword={keyword} color={color} />
          ))}
        </div>
      )}
    </div>
...
function KeywordBadge({ keyword, color }: { keyword: ExtractedKeyword; color: string }) {
  return (
    <div
      ...
      title={`Weight: ${keyword.weight}/100, Frequency: ${keyword.frequency}`}
    >
      {keyword.term}
      <span style={{ fontSize: '9px', color: '#6e6e73' }}>
        {Math.round(keyword.weight)}
      </span>
    </div>
  );
}
...
function calculateMatch(
  ...
  const profileSkills = [
    ...profile.technicalSkills.map((s) => s.name.toLowerCase()),
    ...profile.softSkills.map((s) => s.name.toLowerCase()),
    ...profile.tools.map((t) => t.name.toLowerCase()),
  ];

  const jobKeywords = analysis.extractedKeywords.map((k) => k.term.toLowerCase());
  ...
  return {
    ...
    keywordMatch: {
      matched: analysis.extractedKeywords.filter((k) =>
        profileSkills.some((s) => s.includes(k.term.toLowerCase()) || k.term.toLowerCase().includes(s))
      ),
