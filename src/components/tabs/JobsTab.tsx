...
import { Briefcase, FileText, Download, Trash2, Sparkles, Calendar, MapPin, Building2, Check, Loader2, Scan } from 'lucide-react';
...
export function JobsTab({ panelWidth = 400 }: JobsTabProps) {
  ...
  const handleDownloadDOCX = async (resume: GeneratedResume) => {
  ...
  const handleAnalyzeCurrentPage = async () => {
    try {
      ...
      const keywords = extractKeywordsFromJobDescription(response.data.description);
      const requirements = categorizeJobRequirements(response.data.description);
            ...
            >
              ...
              >
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: '700', color: '#1d1d1f' }}>
                    {generatedResume.atsOptimization.formatCompliance}%
                  </div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '12px', color: '#6e6e73', marginBottom: '4px' }}>Content</div>
                  <div style={{ fontSize: '20px', fontWeight: '700', color: '#1d1d1f' }}>
                    {generatedResume.atsOptimization.contentQuality}%
                  </div>
                </div>
              </div>
