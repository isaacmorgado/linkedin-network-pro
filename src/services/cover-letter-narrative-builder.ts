 ...
 */
const CLOSING_THEMES: ClosingThemePattern[] = [
  {
...
function scorePatternFit(
  ...
  const availableData = {
    ...
    company_focus: jobContext.keyRequirements?.[0]?.term ? 1 : 0,
    current_title: profile.title ? 1 : 0,
    mission: jobContext.keyRequirements?.[0]?.term ? 1 : 0,
    technical_area: jobContext.keyRequirements?.[1]?.term ? 1 : 0,
    ...
    vision: jobContext.keyRequirements?.[0]?.term ? 1 : 0,
...
function fillHookTemplate(
  ...
  const replacements: Record<string, string> = {
    ...
    company_focus: jobContext.keyRequirements?.[0]?.term || 'innovation',
    current_title: profile.title,
    mission: jobContext.keyRequirements?.[0]?.term || 'creating impact',
    technical_area:
      jobContext.keyRequirements?.[1]?.term ||
      jobContext.keyRequirements?.[0]?.term ||
    ...
    vision: jobContext.keyRequirements?.[0]?.term || 'innovation',
  };
...
function fallbackHook(
  ...
  switch (profile.metadata.seniority) {
    ...
    case 'mid':
    case 'senior':
      return `With my background in ${topDomain}, I'm drawn to ${jobContext.company}'s focus on ${jobContext.keyRequirements?.[0]?.term || 'innovation'}.`;

    case 'staff':
    case 'principal':
      return `Throughout my career building teams and systems in ${topDomain}, I've been impressed by ${jobContext.company}'s approach to ${jobContext.keyRequirements?.[0]?.term || 'technical excellence'}.`;
...
function buildValueProposition(
  ...
  if (topRequirement) {
    return `My experience ${achievementAction} ${achievementObject} demonstrates the ${topRequirement.term} expertise you're seeking.`;
  }
...
function buildConnectionToRole(
  ...
  const requirementTerm = requirement.term;
