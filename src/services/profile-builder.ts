...
function extractObject(bullet: string, action: string): string {
  ...
  for (const stop of stopWords) {
    const index = afterAction.toLowerCase().indexOf(stop);
    if (index > 0) {
      object = afterAction.substring(0, index);
      break;
    }
  }
 ...
 */
function extractMetrics(bullet: string): typeof import('../types/resume-tailoring').Metric[] {
  const metrics: typeof import('../types/resume-tailoring').Metric[] = [];
...
function convertVolunteer(entry: VolunteerFormEntry): typeof import('../types/resume-tailoring').VolunteerExperience {
  const achievements = (entry.bullets || []).map((bullet, i) =>
