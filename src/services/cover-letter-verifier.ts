...
export function verifyCoverLetter(
  ...
  const personalization = checkPersonalization(coverLetter, jobContext);
...
function verifyClaimAgainstProfile(
  ...
  switch (claim.type) {
    ...
    case 'achievement':
      ...
      const hasCompany = Array.from(profileFacts.companies).some((c) =>
        achievementText.includes(c)
      );
...
function checkKeywordCoverage(
  ...
  for (const keyword of jobContext.keyRequirements) {
    ...
    if (searchTerms.some((term) => new RegExp(`\\b${term}\\b`).test(lowerLetter))) {
      keywordsUsed.push(keyword.term);
    }
  }
