 ...
 */
type MatchType = 'direct' | 'semantic' | 'transferable' | 'inferred';
...
export function matchUserToJob(profile: UserProfile, requirements: JobRequirements): MatchReport {
  ...
  try {
    ...
    const matchedRequirements = new Set(allMatches.map(m => m.requirement.term.toLowerCase()));
    const missingRequired = requirements.required.filter(
      req => !matchedRequirements.has(req.term.toLowerCase())
    );
    ...
    log.info(LogCategory.SERVICE, 'Missing requirements identified', {
      ...
      criticalMissing: missingRequired.filter(r => r.required).map(r => r.term),
    });
...
function matchRequirements(
  ...
  for (const requirement of requirements) {
    ...
    if (match) {
      ...
      log.debug(LogCategory.SERVICE, `Match found for "${requirement.term}"`, {
      ...
      log.debug(LogCategory.SERVICE, `No match found for "${requirement.term}"`, {
        required: isRequired,
      });
    }
  }
...
function findMatchForRequirement(
  ...
  const reqLower = requirement.term.toLowerCase();
...
function findDirectMatch(
  ...
  if (userSkills.has(reqLower)) {
    const evidence = findEvidenceForSkill(requirement.term, profile);
    ...
    if (evidence.length > 0) {
      return {
        ...
        matchType: 'direct',
        confidence: 1.0,
        explanation: `Direct match: User explicitly lists "${requirement.term}" as a skill with ${evidence.length} concrete examples`,
      };
    }
  }
      ...
      if (userSkills.has(synonym.toLowerCase())) {
        ...
        if (evidence.length > 0) {
          return {
            ...
            matchType: 'direct',
            confidence: 0.95,
            explanation: `Direct match via synonym: User has "${synonym}" which matches "${requirement.term}"`,
          };
        }
      }
    }
  ...
  if (similarSkills.length > 0) {
    ...
    if (evidence.length > 0) {
      ...
      return {
        ...
        matchType: 'semantic',
        confidence,
        explanation: `Semantic match: User has "${bestMatch.skill}" which is related to "${requirement.term}" (${(bestMatch.similarity * 100).toFixed(0)}% similar)`,
      };
    }
  }
      ...
      if (isTransferable) {
        ...
        if (evidence.length > 0) {
          return {
            ...
            confidence: 0.6,
            explanation: `Transferable skill: User's "${sourceSkill}" experience demonstrates "${requirement.term}" (career changer strength)`,
          };
        }
      }
    }
      ...
      if (isImplied) {
        ...
        if (evidence.length > 0) {
          return {
            ...
            matchType: 'inferred',
            confidence: 0.7,
            explanation: `Inferred skill: Since user knows "${parentSkill}", they likely know "${requirement.term}"`,
          };
        }
      }
    }
...
function generateRecommendations(
  ...
  for (const skill of missingRequired.slice(0, 3)) { // Top 3
    recommendations.push({
      ...
      priority: 'high',
      skill: skill.term,
      reason: `Critical missing skill: "${skill.term}" is a required qualification`,
      suggestion: `Consider gaining experience with "${skill.term}" through projects, courses, or professional work. This is a must-have for this role.`,
    });
  }
  ...
  for (const skill of missingPreferred.slice(0, 2)) { // Top 2
    recommendations.push({
      ...
      priority: 'medium',
      skill: skill.term,
      reason: `Preferred skill missing: "${skill.term}" would strengthen your application`,
      suggestion: `Learning "${skill.term}" could make you a more competitive candidate. Consider adding this to your skill development plan.`,
    });
  }
  ...
  for (const match of weakMatches.slice(0, 2)) {
    recommendations.push({
      ...
      priority: 'medium',
      skill: match.requirement.term,
      reason: `Weak match for "${match.requirement.term}" (${match.matchType} match)`,
      suggestion: `Emphasize your "${match.requirement.term}" experience more prominently in your resume. Add more specific examples.`,
    });
  }
  ...
  const missingTechnicalSkills = missing.filter(m =>
    m.category === 'technical-skill' || m.category === 'certification'
  );
  for (const skill of missingTechnicalSkills.slice(0, 1)) {
    recommendations.push({
      ...
      priority: 'low',
      skill: skill.term,
      reason: `Technical certification could validate "${skill.term}" skills`,
      suggestion: `Consider pursuing a certification in "${skill.term}" to demonstrate competency and fill this gap.`,
    });
  }
