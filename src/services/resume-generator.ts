...
function selectRelevantExperience(
  profile: UserProfile,
  matchReport: typeof import('./resume-matcher').MatchReport,
...
function getTopKeywords(requirements: JobRequirements, count: number): string[] {
  ...
  const required = requirements.required.map(k => k.term);
  const preferred = requirements.preferred.map(k => k.term);
...
async function rewriteExperienceSections(
  experiences: typeof import('../types/resume-tailoring').WorkExperience[],
  ...
  for (const exp of experiences) {
    ...
    const bullets = rewrittenBullets.map((rb, idx) => {
    ...
    tailored.push({
      ...
      bullets,
      originalBullets: exp.achievements.map(a => a.bullet),
...
async function rewriteProjectSections(
  projects: typeof import('../types/resume-tailoring').Project[],
  targetKeywords: string[],
  config?: Partial<ResumeGenerationConfig>
): Promise<TailoredSection[]> {
  const tailored: TailoredSection[] = [];
  ...
  for (const project of projects) {
    ...
    tailored.push({
      ...
      bullets,
      originalBullets: project.achievements.map(a => a.bullet),
      changes: rewrittenBullets,
    });
  }
...
function generateSkillsSection(
  ...
  const matchedSkills = new Set(
    matchReport.matches.map(m => m.requirement.term)
  );
  ...
  for (const skill of profile.skills) {
    ...
    if (isMatched) {
      categorized[category].unshift(skill.name);
    } else {
...
function calculateATSScore(
  ...
  const requiredKeywords = requirements.required.map(k => k.term.toLowerCase());
  const preferredKeywords = requirements.preferred.map(k => k.term.toLowerCase());
