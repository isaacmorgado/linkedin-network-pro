...
import { rewriteBullet, rewriteBulletsBatch } from './bullet-rewriter';
...
async function example4_BatchProcessing() {
  ...
  try {
    ...
    const totalKeywordsInjected = results.reduce((sum, r) => sum + r.keywordsAdded.length, 0);
    const hallucinations = results.filter(r => !r.factVerification.allFactsPreserved).length;
