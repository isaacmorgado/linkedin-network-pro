...
describe('findUniversalConnection - Minimal Profile Handling', () => {
  it('should default to Stage 5 (no recommendation) with minimal profile', async () => {
    ...
    expect(strategy.type).toBe('none'); // Stage 5
  ...
  it('should provide helpful guidance for minimal profiles', async () => {
    ...
    expect(strategy.reasoning).toContain('Low profile similarity');
  ...
  it('should handle minimal profile with location (slight improvement)', async () => {
    ...
    // Still likely Stage 5, but confidence might be slightly higher
