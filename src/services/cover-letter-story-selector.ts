...
function extractSituation(achievement: Achievement): string {
  ...
  const objectWords = achievement.object?.toLowerCase() || '';
...
function countDirectMatches(
  ...
  return jobKeywords.filter((kw) =>
    achievementText.includes(kw.term.toLowerCase())
  ).length;
}
...
function countSemanticMatches(
  ...
  return jobKeywords.filter((kw) => {
    ...
    const variations = [kw.term, ...(kw.synonyms || [])].map((s) =>
      s.toLowerCase()
    );
 ...
 */
function isRecent(achievement: Achievement): boolean {
...
function calculateSTARCompleteness(star: StarFramework): number {
  let completeness = 0;
  const componentCount = 4; // Situation, Task, Action, Result
...
function extractKeywords(
  ...
  const achievementKeywords = achievement.keywords || [];
  const jobTerms = jobContext.keyRequirements.map((r) => r.term.toLowerCase());

  // Filter achievement keywords that match job requirements
  return achievementKeywords.filter((ak) =>
    jobTerms.some((jt) => jt.includes(ak.toLowerCase()) || ak.toLowerCase().includes(jt))
  );
}
